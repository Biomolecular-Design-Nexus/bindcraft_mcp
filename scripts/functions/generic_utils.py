...
def check_n_trajectories(design_paths, advanced_settings):
    ...
    if advanced_settings["max_trajectories"] is not False and len(n_trajectories) >= advanced_settings["max_trajectories"]:
        logger.info(f"Target number of {len(n_trajectories)} trajectories reached, stopping execution...")
        return True
    else:
        return False
...
def check_accepted_designs(design_paths, mpnn_csv, final_labels, final_csv, advanced_settings, target_settings, design_labels):
    ...
    if len(accepted_binders) >= target_settings["number_of_final_designs"]:
        logger.info(f"Target number {len(accepted_binders)} of designs reached! Reranking...")
...
def check_jax_gpu():
    ...
    if not has_gpu:
        logger.error("No GPU device found, terminating.")
        exit()
    else:
        logger.info(f"Found {len(devices)} JAX device(s):")
        for i, device in enumerate(devices):
            logger.info(f"  {device.device_kind}{i + 1}: {device.platform}")

# check all input files being passed
def perform_input_check(args):
    # Get the directory of the scripts folder (parent of functions)
    functions_dir = os.path.dirname(os.path.abspath(__file__))
    scripts_dir = os.path.dirname(functions_dir)
    ...
    if not args.filters:
        # Look for default filters in scripts/settings_filters or repo/BindCraft/settings_filters
        default_filters = os.path.join(scripts_dir, 'settings_filters', 'default_filters.json')
        if not os.path.exists(default_filters):
            # Fallback to repo path
            repo_dir = os.path.join(os.path.dirname(scripts_dir), 'repo', 'BindCraft')
            default_filters = os.path.join(repo_dir, 'settings_filters', 'default_filters.json')
        args.filters = default_filters

    # Set a random advanced json settings file if not provided
    if not args.advanced:
        # Look for default advanced in scripts/settings_advanced or repo/BindCraft/settings_advanced
        default_advanced = os.path.join(scripts_dir, 'settings_advanced', 'default_4stage_multimer.json')
        if not os.path.exists(default_advanced):
            # Fallback to repo path
            repo_dir = os.path.join(os.path.dirname(scripts_dir), 'repo', 'BindCraft')
            default_advanced = os.path.join(repo_dir, 'settings_advanced', 'default_4stage_multimer.json')
        args.advanced = default_advanced

    return args.settings, args.filters, args.advanced

# check specific advanced settings
def perform_advanced_settings_check(advanced_settings, scripts_folder):
    # Get the functions directory path (where this file is located)
    functions_dir = os.path.dirname(os.path.abspath(__file__))

    # set paths to model weights and executables
    if scripts_folder == "colab":
        ...
        if not advanced_settings["af_params_dir"]:
            # Use the scripts folder as the base for AF2 params
            advanced_settings["af_params_dir"] = scripts_folder
        if not advanced_settings["dssp_path"]:
            # First try the scripts/functions directory
            dssp_path = os.path.join(functions_dir, 'dssp')
            advanced_settings["dssp_path"] = dssp_path
        if not advanced_settings["dalphaball_path"]:
            # First try the scripts/functions directory
            dalphaball_path = os.path.join(functions_dir, 'DAlphaBall.gcc')
            advanced_settings["dalphaball_path"] = dalphaball_path
    ...
    with zipfile.ZipFile(zip_filename, 'a', zipfile.ZIP_DEFLATED) as zipf:
        for file in os.listdir(folder_path):
            if file.endswith(extension):
                ...
                os.remove(file_path)
    logger.info(f"Files in '{folder_basename}' have been zipped and removed.")
