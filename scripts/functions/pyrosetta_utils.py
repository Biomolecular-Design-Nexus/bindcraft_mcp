...
def score_interface(pdb_file, binder_chain="B"):
    logger.info(f"[PyRosetta] Scoring interface for {os.path.basename(pdb_file)}")
...
def pr_relax(pdb_file, relaxed_pdb_path):
    if not os.path.exists(relaxed_pdb_path):
        logger.info(f"[PyRosetta] Running FastRelax on {os.path.basename(pdb_file)}")
        pose = pr.pose_from_pdb(pdb_file)
        ...
        logger.info(f"[PyRosetta] Relaxed structure saved to {os.path.basename(relaxed_pdb_path)}")
